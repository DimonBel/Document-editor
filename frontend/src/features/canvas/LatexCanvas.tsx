...
export function LatexCanvas({ addElement, sendCursor, sendPreview }: LatexCanvasProps) {
  const stageRef = useRef<Konva.Stage | null>(null);
  const { width, height } = useWindowSize();

  const elements = useLatexStore((s) => s.elements);
  const liveElement = useLatexStore((s) => s.liveElement);
  const activeTool = useLatexStore((s) => s.activeTool);
  const remotePreviews = useCollabStore((s) => s.remotePreviews);
