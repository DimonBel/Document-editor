...
export function LaTeXEditorPage({ onBack }: Props) {
  ...
  const savePdf = useCallback(() => {
    ...
    iframeRef.current?.contentWindow?.postMessage({ type: 'ltx-compile', src: source }, '*'); // nosemgrep: javascript.browser.security.wildcard-postmessage-configuration.wildcard-postmessage-configuration -- srcdoc iframe origin is null
    iframeRef.current?.contentWindow?.postMessage({ type: 'ltx-get-html' }, '*'); // nosemgrep: javascript.browser.security.wildcard-postmessage-configuration.wildcard-postmessage-configuration -- srcdoc iframe origin is null
  }, [source]);
  ...
  const compile = useCallback(() => {
    ...
    if (iframe?.contentWindow) {
      iframe.contentWindow.postMessage({ type: 'ltx-compile', src: source }, '*'); // nosemgrep: javascript.browser.security.wildcard-postmessage-configuration.wildcard-postmessage-configuration -- srcdoc iframe origin is null
    }
  }, [source, status]);
